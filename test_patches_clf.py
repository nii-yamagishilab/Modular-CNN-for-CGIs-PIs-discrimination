...
-----------------------------------------------------
Script for evaluating the network on patches dataset using the LDA classifier
...
if __name__ == '__main__':
    ...
    text_test = open(os.path.join(model_path, 'test_clf_patches_test_360p.csv'), 'w')
    ...
    output_pred = clf.predict(features_lst)
    output_prob = clf.predict_proba(features_lst)

    acc = metrics.accuracy_score(tol_label, output_pred)
    print('Val accuracy: %.4f' % (acc))

    for i in range(output_pred.shape[0]):
        text_test.write('%d,%.2f\n' % (tol_label[i], output_pred[i]))
